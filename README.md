...
| CNVkit, ASCAT, Control-FREEC, MSIsensor2, LoFreq, Varlociraptor | — not ported | — | the remaining somatic callers that exist in upstream 3.10.0 (tools enum + `bam_variant_calling_somatic_all`); tumor/normal pairs required; further callers follow the same `[[pairs]]` pattern once their env fixtures exist. **SomaticSniper and VarDict are not upstream 3.10.0 features** (absent from the tools enum and every wired subworkflow) and are therefore not listed |
