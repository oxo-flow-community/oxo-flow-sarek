...
| FreeBayes/mpileup somatic and tumor-only modes | — not ported | — | upstream `somatic_all`/`tumor_only_all` FreeBayes+mpileup legs; the port's `call_freebayes`/`call_mpileup` are germline single-sample, and the tumor-only path is Mutect2 only |
